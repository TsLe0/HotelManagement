...
<head>
    ...
    <style>
        .details-card { background-color: #fff; border-radius: 8px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); padding: 2rem; }
        .details-header { border-bottom: 1px solid #e5e7eb; padding-bottom: 1rem; margin-bottom: 1.5rem; }
        .details-grid { display: grid; grid-template-columns: repeat(1, 1fr); gap: 1.5rem; }
        @media (min-width: 768px) { .details-grid { grid-template-columns: repeat(2, 1fr); } }
        .detail-item { display: flex; flex-direction: column; }
        .detail-item-label { font-size: 0.875rem; color: #6b7280; margin-bottom: 0.25rem; }
                    ...
                    <div class="details-card max-w-4xl mx-auto">
                        ...
                        <div class="mt-8 pt-5 border-t flex justify-end gap-3">
                            <c:if test="${booking.status ne 'Cancelled' and booking.status ne 'Checked-out'}">
                                ...
                                <a href="${pageContext.request.contextPath}/assign-room?bookingId=${booking.bookingId}" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-md transition duration-300">
